...
useMapEvents({
  click: (e) => {
    ...
    // Update user guess and log for debugging
    const newGuess = {
    ...
    // Set user guess without using setTimeout
    setUserGuess(newGuess);
    
    // Debug check to verify button is still visible
    console.log("Checking button after map click");
    setTimeout(() => {
      const buttonElement = document.getElementById('main-lock-button');
      console.log("Button after map click:", buttonElement ? "exists" : "missing");
    }, 100);
  }
});
...
// Update the MapControls component to position controls above the map
// function MapControls() {
//   const map = useMap();

//   return (
//     <div className="absolute -top-10 left-0 right-0 z-[1001] flex justify-center space-x-2">
//       <button 
//         onClick={() => map.zoomIn()} 
//         className="w-8 h-8 bg-white flex items-center justify-center rounded shadow hover:bg-gray-100 focus:outline-none"
//         title="Zoom in"
//       >
...
// Fixed: Add an effect to ensure the button exists after state changes
useEffect(() => {
...
return (
  <div className="relative h-screen w-full overflow-hidden">
    ...
    {/* Map overlay with controls positioned outside the MapContainer */}
    {/* Map overlay with controls positioned at the left upper corner */}
    {showMap && panoramaLoaded && (
      <div 
        className="absolute rounded-lg shadow-lg overflow-hidden"
        style={{
          width: '300px',
          height: '225px', // Back to original height
          right: '20px',
          bottom: '100px',
          zIndex: 1001,
          position: 'absolute'
    ...
    {/* FIXED: Main Lock In Button - Centered at bottom, with higher z-index */}
    {/* Using visibility instead of conditional rendering to prevent unmounting/remounting */}
    <div
      id="main-lock-button-container"
      style={{
        position: 'fixed',
        bottom: '3rem',     // Distance from bottom of viewport
        right: '1rem',      // Distance from right of viewport
        display: 'flex',
        zIndex: 9999,
        pointerEvents: 'auto'
      }}
    >
      <button
        ...
        className={`
          ...
          ${userGuess ? 'bg-red-600 hover:bg-red-700 cursor-pointer' : 'bg-gray-500 cursor-not-allowed'}
        `}
        style={{
          position: 'relative',
