...
import { InputNumber, Button, message, Card, Typography, Flex } from "antd";
...
const LobbyPage: React.FC = () => {
  ...
  const subscribeToLobbyTopics = useCallback(
    (lobbyId: number) => {
      console.log("[subscribeToLobbyTopics] lobbyId =", lobbyId);
      if (!stompClient.current) return;

      // config updates & disbanded
      updateSub.current = stompClient.current.subscribe(`/topic/lobby/${lobbyId}`, (msg) => {
      ...
      // user join/leave
      usersSub.current = stompClient.current.subscribe(`/topic/lobby/${lobbyId}/users`, (msg) => {
        ...
        if (type === "USER_JOINED") {
          ...
          setJoinedUsers((curr) =>
            curr.some((u) => u.userid === newId) ? curr : [...curr, { username, userid: newId }]
          );
        }
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        joinResultSub.current = client.subscribe("/user/topic/lobby/join/result", (msg) => {
  ...
  }, [user, lobbyCode, router, subscribeToLobbyTopics]);
